...
const Cell = create_ssr_component(($$result, $$props, $$bindings, slots) => {
	...
	: ``}${value
	? `<text class="${"value svelte-1veput"}" x="${"0.5"}" y="${"0.9"}" dominant-baseline="${"auto"}" text-anchor="${"middle"}">${escape(value)}</text>`
	: ``}<text class="${"number svelte-1veput"}" x="${"0.1"}" y="${"0.1"}" dominant-baseline="${"hanging"}" text-anchor="${"start"}">${escape(number)}</text></g>`;
});
